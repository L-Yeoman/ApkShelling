...
void save_dex_file(u_int8_t *data, size_t length) {
    ...
    if (p != NULL) {
        LE("%s file exist",dname);
        return;
    }
    LE( "dump dex file name is : %s", dname);
    LE( "start dump");
    int dex = open(dname, O_CREAT | O_WRONLY, 0644);
    if (dex < 0) {
        LE( "open or create file error");
        return;
    }
    int ret = (int) write(dex, data, length);
    if (ret < 0) {
        LE( "write file error:%d",ret);
    } else {
        LE( "dump dex file success `%s`", dname);
    }
    close(dex);
}
